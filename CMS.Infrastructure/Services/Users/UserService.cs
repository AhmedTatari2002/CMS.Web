...
namespace SMS.Infrastructure.Services.Users
{
    public class UserService : IUserService
    {
        ...
        public async Task<string> Create(CreateUserDto dto)
        {
            ...
            user.CreatedBy = dto.FullName;
            user.FCMToken = "default_token_value";
            user.UpdatedBy = "null";
            user.UpdatedAt = null;
            ...
            if (!result.Succeeded)
            {
                    ...
                    {
                        Console.Error.WriteLine($"Code: {error.Code}, Description: {error.Description}");
                    }
        ...
        private string GenratePassword()
        {
            ...
            //string guid =  Guid.NewGuid().ToString().Substring(1, 8);
            //string uppercaseLetter = "A"; // يمكن تعديلها إلى حرف كبير آخر إذا رغبت

            //// دمج الحرف الكبير مع الـ GUID لضمان احتواء كلمة المرور على حرف كبير
            //string password = uppercaseLetter + guid;
            //return password;
        }
    }
}
